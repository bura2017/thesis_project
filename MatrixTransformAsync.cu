...
int matrixTransformAsync(Matrix &matrix, const int row, const int col, const int threads, data_async &data0, data_async &data1) {
  ...
  CHECK_CUDA (cudaMemcpy (data0.dev_col, &(matrix.e[0 + col * matrix.m]), sizeof(float) * matrix.rows,
      cudaMemcpyHostToDevice));
  CHECK_CUDA (cudaMemcpy (data1.dev_col, &(matrix.e[0 + col * matrix.m]), sizeof(float) * matrix.rows,
      cudaMemcpyHostToDevice));
